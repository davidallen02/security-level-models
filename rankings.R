...
dat <- readr::read_csv('./recommendations.txt', col_types = 'cDnnnnnl') %>%  # Generated by outside process
  ...
  dplyr::distinct(TICKER, .keep_all = TRUE) %>%  # Keeping only the most recent recommendation
  dplyr::filter(TICKER %in% buylist) %>% # Removing recommendations for securities no longer on buy list
  dplyr::filter(!SOURCE) %>%  # Removing buy list securities currently with a SOURCE recommendation
  dplyr::mutate(
