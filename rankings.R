...
buylist <- readr::read_csv("//server1/research/david/projects/buy-list/buy-list-equities.csv", col_types = 'c') %>% dplyr::pull()
...
dat <- readr::read_csv("R:/David/Projects/recommendations-and-ranges/data/recommendations.txt",
                       col_types = "cDnnnnnl") %>%
  ...
  dplyr::mutate(
    ...
    GICS_SECTOR_NAME = ifelse(TICKER == "csco", "Information Technology", GICS_SECTOR_NAME)
  ) %>% ## Change this back when CSCO has a GICS_SECTOR_NAME_AGAIN!!!
