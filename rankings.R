...
dat <- readr::read_csv('./recommendations.txt', col_types = 'cDnnnnnl') %>%  # Generated by outside process
  ...
  dplyr::mutate(
    ...
    GROWTH_RANK            = GROWTH_SCORE %>% rank(ties.method = 'first') %>% RankMap(SECTOR_PEERS) %>% multiply_by(!SOURCE),
    CONSERVATIVE_RANK      = CONSERVATIVE_SCORE %>% rank(ties.method = 'first') %>% RankMap(SECTOR_PEERS) %>% multiply_by(!SOURCE),
    
    # Establish target/max weight in sector model using target for overall portfolio weight
    TARGET_WEIGHT     = 5  %>% divide_by(GICS_SECTOR_WEIGHT) %>% multiply_by(100) %>% min(100) %>% multiply_by(!SOURCE),
    MAX_WEIGHT        = 10 %>% divide_by(GICS_SECTOR_WEIGHT) %>% multiply_by(100) %>% min(100) %>% multiply_by(!SOURCE)
  )
...
dat_conservative <- dat %>%
  dplyr::arrange(GICS_SECTOR_NAME, desc(CONSERVATIVE_RANK)) %>%
  dplyr::mutate(
    CUM_WEIGHT        = cumsum(TARGET_WEIGHT),
    CUM_WEIGHT_ADJ    = dplyr::case_when(
      CUM_WEIGHT > 100 ~ CUM_WEIGHT - 100,
